...
public class Proton extends Circles{
    ...
    protected void calculateTimerPositions(){
        this.timerX = this.getX() + (float)(Variables.PROTON_RADIUS * Math.sin(Math.toRadians(360) * (lifeSpan/Variables.PROTON_LIFESPAN)));
        this.timerY = this.getY() + (float)(Variables.PROTON_RADIUS * Math.cos(Math.toRadians(360) * (lifeSpan/Variables.PROTON_LIFESPAN)));
    }
